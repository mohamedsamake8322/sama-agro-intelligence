    ...
    if st.session_state.user is None:
        ...
        if current_page == 'home':
            ...
            try:
                ...
                print("✅ Loaded marketplace_page")
                marketplace_page()
            except Exception as e:
                st.error(f"Erreur lors du chargement de Marketplace : {e}")

        elif current_page == 'my_products':
            try:
                my_products_page()
            except Exception as e:
                st.error(f"Erreur dans My Products : {e}")

        elif current_page == 'messages':
            try:
                from pages.messages import messages_page
                print("✅ Loaded messages_page")
                messages_page()
            except Exception as e:
            ...
            try:
                ...
                print("✅ Loaded analytics_page")
                analytics_page()
            except Exception as e:
                st.error(f"Erreur dans la page Analytics : {e}")

        elif current_page == 'weather':
            try:
                from pages.weather import weather_page
                print("✅ Loaded weather_page")
                weather_page()
            except Exception as e:
                st.error(f"Erreur dans la page Météo : {e}")

        elif current_page == 'profile':
            try:
                from pages.profile import profile_page
                print("✅ Loaded profile_page")
                profile_page()
            except Exception as e:
                st.error(f"Erreur dans la page Profil : {e}")

        elif current_page == 'payment':
            try:
                payment_page()
            except Exception as e:
                st.error(f"Erreur dans la page Paiement : {e}")
