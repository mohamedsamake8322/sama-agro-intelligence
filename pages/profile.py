...
def display_user_settings():
    ...
    with st.expander("🔐 Privacy Settings"):
        ...
        profile_public = st.checkbox("Make profile public", value=True)
        show_location = st.checkbox("Show location in profile", value=True)
        show_contact = st.checkbox("Allow other users to contact me", value=True)
        
        st.write("**Data Sharing**")
        analytics_sharing = st.checkbox("Share anonymous usage data for platform improvement", value=True)
